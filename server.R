...
### 2. Reactive data frames ----
  ...
  maxn_data <- reactive({
    ...
    if (input$maxn.campaignid.selector == "All") {
      ...
      filter(maxn.data, campaignid == as.character(campaign.name))
    }
  })
...
### 3. Drop downs for species plots ----
  ...
  output$maxn.genus.selector <- renderUI({
    ...
    if (input$maxn.campaignid.selector == "All") {
      ...
      create_dropdown("maxn.genus.selector", choices=options, "Genus:")
    }
  })
  ...
  output$length.genus.selector <- renderUI({
    ...
    if (input$length.campaignid.selector == "All") {
      ...
      options <- df %>%
        ...
        pull("genus")%>%
        sort()
      create_dropdown("length.genus.selector", choices=options, "Genus:")
  ...
  output$maxn.species.selector <- renderUI({
    ...
    if (input$maxn.campaignid.selector == "All") {
      ...
      options <- df %>%
        filter(family == input$maxn.family.selector,genus == input$maxn.genus.selector) %>%
  ...
  output$length.species.selector <- renderUI({
    ...
    if (input$length.campaignid.selector == "All") {
      ...
      create_dropdown("length.species.selector", choices=options, "Species:")
...
output$overall.abundance <- renderText({ 
  paste(sum(maxn.summary.data()$'Total abundance'))
})

output$species.richness <- renderText({ 
  paste(length(unique(maxn.summary()$scientific)))
})
...
output$length.metric.status <- renderPlot({
  ...
  length.metric<-length_metric_data()%>%
    as.data.frame()%>%
...
### 9. Habitat plots ----
  # Habitat pie spatial plot ----
  output$habitat.spatial.plot <- renderLeaflet({
    
    data<-habitat_data()[,c("macroalgae","stony.corals","octocoral.black","sponges","hydroids","consolidated","unconsolidated")]
